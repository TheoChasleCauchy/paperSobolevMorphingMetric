...
def make_table(results_dir, models):
    ...
    print("Generating Embeddings Regularity table.")
        ...
        for model_name in models:
            ...
            if os.path.exists(null_csv_path):
                with open(null_csv_path, "r") as csvfile:
                    ...
                    null_mean_std_str = rows[-1][1]
                    matches = re.findall(r"[-+]?\d*\.\d+(?:[eE][-+]?\d+)?", null_mean_std_str)
                    null_mean, _ = map(float, matches)
            else:
