...
def make_table():
    ...
    def get_metrics_values(results_dir):
        ...
        with open(clap_smoothness_clap_csv_path, 'r') as f:
            ...
            mean_smoothness_clap_corr, _ = map(float, mean_std_clap)
            metrics_values["CLAP Smoothness MF"] = mean_smoothness_clap_corr
        ...
        with open(mert_smoothness_clap_csv_path, 'r') as f:
            ...
            mean_std_clap = re.findall(r"[-+]?\d*\.\d+(?:[eE][-+]?\d+)?", value_string)
            mean_smoothness_clap_corr, _ = map(float, mean_std_clap)
            metrics_values["MERT Smoothness MF"] = mean_smoothness_clap_corr
        ...
        with open(sobolev_k0_p2_csv_path, 'r') as f:
            ...
            mean_sobolev_k0_p2, _ = map(float, mean_std_sobolev_k0_p2)
            metrics_values["Sobolev (0, 2)"] = mean_sobolev_k0_p2
        ...
        with open(sobolev_k1_p2_csv_path, 'r') as f:
            ...
            mean_sobolev_k1_p2, _ = map(float, mean_std_sobolev_k1_p2)
            metrics_values["Sobolev (1, 2)"] = mean_sobolev_k1_p2
        ...
        with open(mfcc_correspondence_csv_path, 'r') as f:
            ...
            mean_correspondence, _ = map(float, mean_std_correspondence)
            metrics_values["MFCC Correspondence SM"] = mean_correspondence
        ...
        with open(mert_correspondence_csv_path, 'r') as f:
            ...
            mean_std_correspondence = re.findall(r"[-+]?\d*\.\d+(?:[eE][-+]?\d+)?", value_string)
            mean_correspondence, _ = map(float, mean_std_correspondence)
            metrics_values["MERT Correspondence SM"] = mean_correspondence
        ...
        with open(cdpam_intermediateness_csv_path, 'r') as f:
            ...
            mean_intermediateness, _ = map(float, mean_std_intermediateness)
            metrics_values["CDPAM Intermediateness SM"] = mean_intermediateness
        ...
        with open(mert_intermediateness_csv_path, 'r') as f:
            ...
            mean_std_intermediateness = re.findall(r"[-+]?\d*\.\d+(?:[eE][-+]?\d+)?", value_string)
            mean_intermediateness, _ = map(float, mean_std_intermediateness)
            metrics_values["MERT Intermediateness SM"] = mean_intermediateness
        ...
        with open(cdpam_smoothness_cdpam_csv_path, 'r') as f:
            ...
            mean_smoothness_cdpam, _ = map(float, mean_std_smoothness_cdpam)
            metrics_values["CDPAM Smoothness SM"] = mean_smoothness_cdpam
        ...
        with open(mert_smoothness_cdpam_csv_path, 'r') as f:
            ...
            mean_std_smoothness_cdpam = re.findall(r"[-+]?\d*\.\d+(?:[eE][-+]?\d+)?", value_string)
            mean_smoothness_cdpam, _ = map(float, mean_std_smoothness_cdpam)
            metrics_values["MERT Smoothness SM"] = mean_smoothness_cdpam
    ...
    results_dir_ref = "exp_invalidate_metrics_fpc/generated/metrics/ref"
    ref_metrics_values = get_metrics_values(results_dir_ref)
    results_dir_null = "exp_invalidate_metrics_fpc/generated/metrics/null"
    ...
    with open(output_csv_path, "w", newline="") as csvfile:
        ...
        header = ["Metric", "Encoder", "Morph", "Null", "NUC", "EQC"]
        ...
        row = [
            "Correspondence", 
            "MFCC",
            f"{ref_metrics_values['MFCC Correspondence SM']:.2f}",
            f"{null_metrics_values['MFCC Correspondence SM']:.2f}",
            f"{nuc_metrics_values['MFCC Correspondence SM']:.2f}",
            f"{eqc_metrics_values['MFCC Correspondence SM']:.2f}"
        ]
        writer.writerow(row)
        row = [
            "Smoothness CLAP",
            "L-CLAP audio",
            f"{ref_metrics_values['CLAP Smoothness MF']:.2f}",
            f"{null_metrics_values['CLAP Smoothness MF']:.2f}",
            f"{nuc_metrics_values['CLAP Smoothness MF']:.2f}",
            f"{eqc_metrics_values['CLAP Smoothness MF']:.2f}"
        ]
        writer.writerow(row)
        row = [
            "Intermediateness",
            "CDPAM",
            f"{ref_metrics_values['CDPAM Intermediateness SM']:.2f}",
            f"{null_metrics_values['CDPAM Intermediateness SM']:.2f}",
            f"{nuc_metrics_values['CDPAM Intermediateness SM']:.2f}",
            f"{eqc_metrics_values['CDPAM Intermediateness SM']:.2f}"
        ]
        writer.writerow(row)
        row = [
            "Smoothness SM",
            "CDPAM",
            f"{ref_metrics_values['CDPAM Smoothness SM']:.2f}",
            f"{null_metrics_values['CDPAM Smoothness SM']:.2f}",
            f"{nuc_metrics_values['CDPAM Smoothness SM']:.2f}",
            f"{eqc_metrics_values['CDPAM Smoothness SM']:.2f}"
        ]
        ...
        row = [
            "Correspondence", 
            "MERT",
            f"{ref_metrics_values['MERT Correspondence SM']:.2f}",
            f"{null_metrics_values['MERT Correspondence SM']:.2f}",
            f"{nuc_metrics_values['MERT Correspondence SM']:.2f}",
            f"{eqc_metrics_values['MERT Correspondence SM']:.2f}"
        ]
        writer.writerow(row)
        row = [
            "Smoothness CLAP",
            "MERT",
            f"{ref_metrics_values['MERT Smoothness MF']:.2f}",
            f"{null_metrics_values['MERT Smoothness MF']:.2f}",
            f"{nuc_metrics_values['MERT Smoothness MF']:.2f}",
            f"{eqc_metrics_values['MERT Smoothness MF']:.2f}"
        ]
        writer.writerow(row)
        row = [
            "Intermediateness",
            "MERT",
            f"{ref_metrics_values['MERT Intermediateness SM']:.2f}",
            f"{null_metrics_values['MERT Intermediateness SM']:.2f}",
            f"{nuc_metrics_values['MERT Intermediateness SM']:.2f}",
            f"{eqc_metrics_values['MERT Intermediateness SM']:.2f}"
        ]
        writer.writerow(row)
        row = [
            "Smoothness SM",
            "MERT",
            f"{ref_metrics_values['MERT Smoothness SM']:.2f}",
            f"{null_metrics_values['MERT Smoothness SM']:.2f}",
            f"{nuc_metrics_values['MERT Smoothness SM']:.2f}",
            f"{eqc_metrics_values['MERT Smoothness SM']:.2f}"
        ]
        writer.writerow(row)
        row = [
            "Sobolev (0, 2)",
            "MERT",
            f"{ref_metrics_values['Sobolev (0, 2)']:.2f}",
            f"{null_metrics_values['Sobolev (0, 2)']:.2f}",
            f"{nuc_metrics_values['Sobolev (0, 2)']:.2f}",
            f"{eqc_metrics_values['Sobolev (0, 2)']:.2f}"
        ]
        writer.writerow(row)
        row = [
            "Sobolev (1, 2)",
            "MERT",
            f"{ref_metrics_values['Sobolev (1, 2)']:.2f}",
            f"{null_metrics_values['Sobolev (1, 2)']:.2f}",
            f"{nuc_metrics_values['Sobolev (1, 2)']:.2f}",
            f"{eqc_metrics_values['Sobolev (1, 2)']:.2f}"
        ]
        writer.writerow(row)
